...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/reminders/remindersSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureRemindersCols';
...
const TableSampleReminders = ({
  ...
  useEffect(() => {
    if (remindersNotify.showNotification) {
      notify(
        remindersNotify.typeNotification,
        remindersNotify.textNotification,
      );
    }
  }, [remindersNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/reminders/reminders-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {reminders && Array.isArray(reminders) && !showGrid && (
        <ListReminders
          reminders={reminders}
          loading={loading}
...
export default TableSampleReminders;
