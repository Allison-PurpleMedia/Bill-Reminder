...
export default {
  ...
  billsManyListFormatter(val) {
    if (!val || !val.length) return [];
    return val.map((item) => item.bill_name);
  },
  billsOneListFormatter(val) {
    if (!val) return '';
    return val.bill_name;
  },
  billsManyListFormatterEdit(val) {
    if (!val || !val.length) return [];
    return val.map((item) => {
      return { id: item.id, label: item.bill_name };
    });
  },
  billsOneListFormatterEdit(val) {
    if (!val) return '';
    return { label: val.bill_name, id: val.id };
  },
